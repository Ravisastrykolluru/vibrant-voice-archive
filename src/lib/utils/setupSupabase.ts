...
export const initializeSupabase = async (): Promise<void> => {
  try {
    ...
    try {
      ...
      await supabase.rpc('create_notification_function' as any) as { data: any, error: any };
      console.log("Created notification functions");
      
      // Create user password update function
      await supabase.rpc('create_password_update_function' as any) as { data: any, error: any };
